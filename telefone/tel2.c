...
if(rank == 0){
	...
	ierr = MPI_Ssend(sendmessage, 6, MPI_CHAR, rank+1, ourtag, MPI_COMM_WORLD);
		...
		//Recebendo informacao da CPU de rank-1 
		ierr = MPI_Recv(getmessage, 6, MPI_CHAR, rank-1, ourtag, MPI_COMM_WORLD, &rstatus);
		printf("Mensagem Chegou ao rank 7\n");
		// Enviando informacao para a CPU 0 novamente
		ierr = MPI_Send(sendmessage, 6, MPI_CHAR, 0 , ourtag, MPI_COMM_WORLD);
		printf("Enviando informacao para a CPU 0\n");	
	}
}
MPI_Finalize();
return 0;
